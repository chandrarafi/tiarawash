...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h3 mb-0 text-gray-800">Edit Pembelian Perlengkapan</h1>
        <p class="mb-0 text-secondary">Perbarui data pembelian perlengkapan</p>
    </div>
        ...
        <div class="card">
            <div class="card-body">
                <form id="formPembelian">
                    <input type="hidden" id="no_faktur_hidden" name="no_faktur" value="<?= $pembelian['no_faktur'] ?>">
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="no_faktur" class="form-label">Nomor Faktur</label>
                                <input type="text" class="form-control" id="no_faktur" name="no_faktur" value="<?= $pembelian['no_faktur'] ?>" required>
                                <div class="invalid-feedback" id="error-no_faktur"></div>
                            </div>
                            <div class="mb-3">
                                <label for="tanggal" class="form-label">Tanggal Pembelian</label>
                                <input type="date" class="form-control" id="tanggal" name="tanggal" value="<?= date('Y-m-d', strtotime($pembelian['tanggal'])) ?>" required>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="supplier" name="supplier" value="<?= $pembelian['supplier'] ?>" required>
                                <div class="invalid-feedback" id="error-supplier"></div>
                            </div>
                            <div class="mb-3">
                                <label for="keterangan" class="form-label">Keterangan</label>
                                <textarea class="form-control" id="keterangan" name="keterangan" rows="3"><?= $pembelian['keterangan'] ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                        ...
                        <button type="submit" class="btn btn-primary" id="btnSimpan">Perbarui Pembelian</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        let noFaktur = $('#no_faktur').val(); // Use no_faktur instead of id

        // Load data perlengkapan dan detail pembelian
        loadPerlengkapan();
        loadDetailPembelian();
        ...
        $('#btnSimpanItem').on('click', function() {
            ...
            const item = {
                ...
                subtotal: subtotal,
                is_new: true // Tandai sebagai item baru
            };
        ...
        $(document).on('change', '.item-jumlah', function() {
            ...
            // Jika item sudah ada di database, tandai untuk diupdate
        ...
        $(document).on('click', '.btn-delete-item', function() {
            ...
            // Jika item sudah ada di database, tandai untuk dihapus
            if (item.id) {
                Swal.fire({
                    title: 'Konfirmasi',
                ...
                // Jika item baru, cukup hapus dari array
        ...
        $('#formPembelian').on('submit', function(e) {
            ...
            const formData = {
                ...
                items: JSON.stringify(items.filter(item => item.is_new)), // Item baru
                updated_items: JSON.stringify(items.filter(item => item.is_updated)) // Item yang diupdate jumlahnya
            };

            // Kirim data ke server
            $.ajax({
                url: '<?= site_url('admin/pembelian/update') ?>',
                type: 'POST',
                data: formData,
                dataType: 'json',
                beforeSend: function() {
                ...
                complete: function() {
                    $('#btnSimpan').prop('disabled', false).html('Perbarui Pembelian');
                }
            });
        });
        ...
        function loadPerlengkapan() {
            console.log('Loading perlengkapan data...');
            $.ajax({
                url: '<?= site_url('admin/pembelian/getPerlengkapan') ?>',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    console.log('Perlengkapan response:', response);
                    console.log('Response length:', response.length);
                    perlengkapanData = response;
                    renderPerlengkapanTable(response);
                },
                error: function(xhr, status, error) {
                    console.error('Error loading perlengkapan:', xhr.responseText);
                    showErrorAlert('Gagal memuat data perlengkapan');
                }
            });
        }

        // Fungsi untuk memuat detail pembelian
        function loadDetailPembelian() {
            console.log('Loading detail pembelian for noFaktur:', noFaktur);
            $.ajax({
                url: '<?= site_url('admin/pembelian/getDetailPembelian/') ?>' + noFaktur,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    console.log('Detail pembelian response:', response);
                    console.log('Detail response length:', response.length);
                    // Reset items array
                    items = [];

                    // Tambahkan detail ke items array
                    response.forEach(function(detail) {
                        items.push({
                            id: detail.id,
                            perlengkapan_id: detail.perlengkapan_id,
                            perlengkapan_text: detail.nama_perlengkapan + ' - ' + detail.kategori,
                            jumlah: parseInt(detail.jumlah),
                            harga_satuan: parseFloat(detail.harga_satuan),
                            subtotal: parseFloat(detail.subtotal),
                            is_new: false // Tandai sebagai item yang sudah ada
                        });
                    });
                ...
                error: function(xhr, status, error) {
                    showErrorAlert('Gagal memuat detail pembelian');
                }
            });
        }
        ...
        function deleteDetailItem(detailId) {
            $.ajax({
                url: '<?= site_url('admin/pembelian/deleteDetail/') ?>' + detailId,
                type: 'DELETE',
                dataType: 'json',
                success: function(response) {
                    if (response.status) {
                        ...
                        // Update total harga
                        totalHarga = response.total_harga;
        ...
        function validateForm() {
            ...
            $('#no_faktur, #tanggal, #supplier').removeClass('is-invalid');
            $('#error-no_faktur, #error-tanggal, #error-supplier').text('');
