...
<head>
    <meta charset="UTF-8">
    <title>Laporan Data Perlengkapan PerBulan</title>
    <style>
        body {
...
<body>
    ...
    <div class="report-title">
        Laporan Perlengkapan<br>PerBulan
    </div>
            ...
            <?php if (!empty($perlengkapan)): ?>
                <?php foreach ($perlengkapan as $index => $item): ?>
                    <tr>
                        ...
                        <td><?= esc($item['id']) ?></td>
                        <td style="text-align: left; padding-left: 6px;"><?= esc($item['nama']) ?></td>
                        <td><?= date('d/m/Y', strtotime($item['created_at'])) ?></td>
                        <td><?= esc($item['kategori']) ?></td>
                        <td><?= esc($item['stok']) ?></td>
                        <td style="text-align: right; padding-right: 6px;"><?= number_format($item['harga'], 0, ',', '.') ?></td>
                        <td style="text-align: left; padding-left: 6px;"><?= esc($item['deskripsi'] ?? '-') ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr>
                    <td colspan="8">Tidak ada data perlengkapan</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
